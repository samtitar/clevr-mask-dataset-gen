...
def render_scene(
    ...
    cam_behind = Vector((0, 0, -1)) @ cam_matrix
    cam_left = Vector((-1, 0, 0)) @ cam_matrix
    cam_up = Vector((0, 1, 0)) @ cam_matrix
    ...
    render_object_mask(blender_objects, output_mask)
...
def check_visibility(blender_objects, min_pixels_per_object):
    ...
    color_count = Counter(
        (int(p[i] * 255), int(p[i + 1] * 255), int(p[i + 2] * 255), int(p[i + 3] * 255))
...
def render_object_mask(blender_objects, mask_path="flat.png"):
    f, path = tempfile.mkstemp(suffix=".png")
    ...
    sizes = {k: i for i, (k, _) in enumerate(color_count.most_common())}
    ...
    new_pixels = [[e, e, e, 1] for e in new_pixels]
    new_pixels = [e for row in new_pixels for e in row]

    img.pixels = new_pixels
    img.save_render(filepath=mask_path)
